...
class AuthorizationWindow:
    ...
    def create_widgets(self):
        ...
        self.canvas.create_rectangle(
            350.0, 200.0, 700.0, 203.0, fill="#000000", outline=""
        )
    ...
    def draw_login_form(self):
        self.canvas.create_text(
            525.0, 30, text="Login:", font=("Inter", 18), fill="#000"
        )

        self.canvas.create_text(
            525.0, 131, text="Password:", font=("Inter", 18), fill="#000"
        )
    ...
    def draw_buttons(self):
        ...
        self.reg_button = Button(
            image=self.reg_button_image,
            command=lambda: [self.window.destroy(), RegistrationWindow(self.db)],
        )
        ...
        self.resset_button = Button(
            image=self.resset_button_image,
            command=lambda: [self.window.destroy(), RessetWindow(self.db)],
        )
    ...
    def verify_user(self):
        ...
        if not user_pass:
            ...
            self.window.destroy()
            DiaryWindow(self.db, user_pass[0])
...
class RegistrationWindow:
    ...
    def draw_input(self):
        ...
        self.username_entry = Entry(
            bd=0, bg="#2C302C", fg="#FFFFFF", highlightthickness=0
        )
        ...
        self.gender_menu = tkinter.OptionMenu(
            self.canvas, self.gender_var, *["Male", "Female"]
        )
        ...
        self.activity_menu = tkinter.OptionMenu(
            self.canvas,
            self.activity_var,
    ...
    def draw_reset(self, password, username):
        ...
        self.reset_button = Button(
            ...
            command=lambda: self.update_password(
                password, self.password_entry.get(), username
            ),
        )
...
class DiaryWindow:
    def __init__(self, db: DataBase, account_id):
        self.window = Tk()
    ...
    def draw_buttons(self):
        ...
        self.acc_button = Button(
            image=self.acc_button_image,
            command=lambda: [
                self.window.destroy(),
                AccountWindow(self.db, self.account_id),
            ],
        )
        ...
        self.stat_button = Button(
            image=self.stat_button_image,
            command=lambda: [
                self.window.destroy(),
                ReportWindow(self.db, self.account_id),
            ],
        )
        self.stat_button.place(x=394, y=5, width=70, height=70)

        # Кнопка избранное
        self.favour_button_image = PhotoImage(
            file=ASSETS_PATH / "Favourites_button.png"
        )
        self.favour_button = Button(
            image=self.favour_button_image,
            command=lambda: [
                self.window.destroy(),
                FavouritesWindow(self.db, self.account_id),
            ],
        )
        ...
        self.diary_button = Button(
            image=self.diary_button_image,
            command=lambda: [
                self.window.destroy(),
                DiaryWindow(self.db, self.account_id),
            ],
        )
        self.diary_button.place(x=54, y=5, width=70, height=70)

        # Кнопка завтрак
        self.breakfast_button_image = PhotoImage(
            file=ASSETS_PATH / "Breakfast_button.png"
        )
    ...
    def breakfast_wind(self):
        DietList(self.db, self.account_id, "Breakfast")

    def lunch_wind(self):
        DietList(self.db, self.account_id, "Lunch")

    def dinner_wind(self):
        DietList(self.db, self.account_id, "Dinner")


class DietList:
    def __init__(self, db: DataBase, account_id, meal_type=""):
    ...
    def draw_entry(self):
        ...
        self.entry_field = Entry(
            self.new_canvas, bd=0, bg="#2C302C", fg="#FFFFFF", highlightthickness=0
        )
    ...
    def draw_buttons(self):
        ...
        self.search_button = Button(
            ...
            command=lambda: self.get_table(),
        )
        self.search_button.place(x=285, y=101, width=117, height=43)

        # Кнопка посмотреть избранное
        self.fav_list_button_image = PhotoImage(
            file=ASSETS_PATH / "fav_list_button.png"
        )
        ...
        self.add_diet_button_image = PhotoImage(
            file=ASSETS_PATH / "add_diet_button.png"
        )
        self.add_diet_button = Button(
            self.new_canvas,
            image=self.add_diet_button_image,
    ...
    def get_table(self):
        ...
        self.favourite_tree.configure(
            yscrollcommand=tkinter.Scrollbar(self.new_window).set
        )
        self.favourite_tree.configure(yscroll=tkinter.YES, xscroll=tkinter.YES)
        self.favourite_tree["columns"] = (
            "id",
            "name",
            "proteins",
            "fats",
        ...
        self.favourite_tree.heading(
            "carbohydrates", text="Carbohydrates", anchor=tkinter.CENTER
        )

        self.favourite_tree.column("kilocalories", anchor=tkinter.CENTER, width=100)
        self.favourite_tree.heading(
            "kilocalories", text="Kilocalories", anchor=tkinter.CENTER
        )

        for product in products:
            self.favourite_tree.insert(
                "",
                "end",
                values=(
                    product[0],
    ...
    def favourite_window(self):
        ...
        # Кнопка добавить в рацион
        self.add_diet_button2_image = PhotoImage(
            file=ASSETS_PATH / "add_diet_button.png"
        )
        self.add_diet_button2 = Button(
            self.third_canvas,
            image=self.add_diet_button2_image,
            command=lambda: self.add_to_diet(),
        )
...
class FavouritesWindow:
    ...
    def draw_buttons(self):
        ...
        self.search_button_image = PhotoImage(file=ASSETS_PATH / "search_button.png")
        self.search_button = Button(
            image=self.search_button_image, command=lambda: self.output_table()
        )
        ...
        self.add_fav_button = Button(
            image=self.add_fav_button_image, command=lambda: self.add_to_favourites()
        )
        ...
        self.fav_list_button = Button(
            image=self.fav_list_button_image,
            command=lambda: FavouriteList(self.db, self.account_id),
        )
...
class FavouriteList:
    ...
    def draw_button(self):
        ...
        self.del_fav_button = Button(
            self.new_canvas,
            image=self.del_fav_button_image,
...
class ReportWindow:
    ...
    def draw_buttons(self):
        ...
        # Кнопка калорий
        self.calories_button_image = PhotoImage(
            file=ASSETS_PATH / "calories_button.png"
        )
    ...
    def draw_background(self):
        ...
        self.canvas.create_rectangle(
            0.0, 146.0, 700.0, 149.0, fill="#000000", outline=""
        )
    ...
    def get_diet_info(self):
        ...
        if meal is not None:
            self.meal_info = [
                tuple(map(float, meal[i]))
                if i < len(meal)
                else (tuple(0 for _ in range(4)))
                for i in range(3)
            ]
        else:
    ...
    def calories_page(self):
        ...
        self.avg_calories = Label(
            self.canvas,
            text=f"{(self.total_num_calories / 3):.3f} kcal",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.avg_calories.place(x=321, y=159, width=338, height=42)

        self.total_num_calories = (
            float(sum(self.kilocalories)) if sum(self.kilocalories) != 0 else 1
        )
        ...
        self.break_calories = Label(
            self.canvas,
            text=f"{(self.kilocalories[0]):.3f} kcal",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.break_calories.place(x=165, y=252, width=94, height=42)

        self.break_calories_perc = Label(
            self.canvas,
        ...
        self.lunch_calories = Label(
            self.canvas,
            text=f"{(self.kilocalories[1]):.3f} kcal",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.lunch_calories.place(x=124, y=353, width=94, height=42)

        self.lunch_calories_perc = Label(
            self.canvas,
        ...
        self.dinner_calories = Label(
            self.canvas,
            text=f"{(self.kilocalories[2]):.3f} kcal",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.dinner_calories.place(x=131, y=453, width=94, height=42)

        self.dinner_calories_perc = Label(
            self.canvas,
        ...
        self.weight = Label(
            self.canvas,
            text=f"{abs(self.difference / 7700):.3f} kg",
            bg="#2C302C",
            fg="#FFFFFF",
        )
    ...
    def macros_page(self):
        ...
        self.avg_pfc_1 = Label(
            self.canvas,
            text=f"{(self.total_num_proteins / 3):.3f} proteins",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.avg_pfc_1.place(x=260, y=159, width=118, height=42)

        self.avg_pfc_2 = Label(
            self.canvas,
            text=f"{(self.total_num_fats / 3):.3f} fats",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.avg_pfc_2.place(x=390, y=159, width=118, height=42)

        self.avg_pfc_3 = Label(
            self.canvas,
            text=f"{(self.total_num_carbohydrates / 3):.3f} carbohydrates",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        ...
        self.break_proteins = Label(
            self.canvas,
            text=f"{(self.proteins[0]):.3f} proteins",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        self.break_proteins.place(x=165, y=252, width=94, height=42)

        self.break_fats = Label(
            self.canvas, text=f"{(self.fats[0]):.3f} fats", bg="#2C302C", fg="#FFFFFF"
        )
        self.break_fats.place(x=270, y=252, width=94, height=42)

        self.break_carbo = Label(
            self.canvas,
            text=f"{(self.carbohydrates[0]):.3f} carbohydrates",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        ...
        self.break_proteins = Label(
            self.canvas,
            text=f"{(self.proteins[1]):.3f} proteins",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        ...
        self.break_fats = Label(
            self.canvas, text=f"{(self.fats[1]):.3f} fats", bg="#2C302C", fg="#FFFFFF"
        )
        self.break_fats.place(x=270, y=352, width=94, height=42)

        self.break_carbo = Label(
            self.canvas,
            text=f"{(self.carbohydrates[1]):.3f} carbohydrates",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        ...
        self.break_proteins = Label(
            self.canvas,
            text=f"{(self.proteins[2]):.3f} proteins",
            bg="#2C302C",
            fg="#FFFFFF",
        )
        ...
        self.break_fats = Label(
            self.canvas, text=f"{(self.fats[2]):.3f} fats", bg="#2C302C", fg="#FFFFFF"
        )
        self.break_fats.place(x=270, y=452, width=94, height=42)

        self.break_carbo = Label(
            self.canvas,
            text=f"{(self.carbohydrates[2]):.3f} carbohydrates",
            bg="#2C302C",
            fg="#FFFFFF",
        )
...
class AccountWindow:
    ...
    def draw_info(self):
        ...
        name_info = Label(
            self.canvas, text=f"{self.account_info[0]}", bg="#2C302C", fg="#FFFFFF"
        )
        ...
        age_info = Label(
            self.canvas, text=f"{self.account_info[5]}", bg="#2C302C", fg="#FFFFFF"
        )
        ...
        sex_info = Label(
            self.canvas, text=f"{self.account_info[1]}", bg="#2C302C", fg="#FFFFFF"
        )
        ...
        activity_info = Label(
            self.canvas, text=f"{self.account_info[2]}", bg="#2C302C", fg="#FFFFFF"
        )
        ...
        weight_info = Label(
            self.canvas, text=f"{self.account_info[3]}", bg="#2C302C", fg="#FFFFFF"
        )
        ...
        height_info = Label(
            self.canvas, text=f"{self.account_info[4]}", bg="#2C302C", fg="#FFFFFF"
        )
    ...
    def draw_buttons(self):
        ...
        self.change_acc_button_image = PhotoImage(
            file=ASSETS_PATH / "change_acc_button.png"
        )
        self.change_acc_button = Button(
            image=self.change_acc_button_image,
            command=lambda: [
                self.window.destroy(),
                ChangeInfo(self.db, self.account_info, self.account_id),
            ],
        )
        ...
        self.del_acc_button = Button(
            image=self.del_acc_button_image, command=lambda: self.delete_account_wind()
        )
        ...
        self.del_DB_button = Button(
            image=self.del_DB_button_image,
...
class ChangeInfo:
    def __init__(self, db: DataBase, user_info, account_id):
        self.window = Tk()
    ...
    def update_info(self):
        if (
            len(self.name_info.get().split()) != 1
            and len(self.age_info.get().split()) != 1
            and len(self.weight_info.get().split()) != 1
            and len(self.height_info.get().split()) != 1
        ):
            ...
            self.db.update_user_info(
                self.age_info.get(),
                self.gender_var.get(),
                self.weight_info.get(),
                self.height_info.get(),
                self.goal_info.get("1.0", "end-1c"),
                self.account_id,
            )
            self.window.destroy()
            AccountWindow(self.db, self.account_id)
